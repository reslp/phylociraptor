...
base_size <- 7
p <- ggplot(mdata, aes(Var2, Var1, fill=value)) +geom_tile() + scale_fill_gradient(low="white", high="orange") +theme(axis.ticks = element_blank(),legend.position="None",axis.text.y = element_text(size=base_size*0.7), axis.text.x = element_text(size = base_size *0.8,angle=-90, hjust = 0,vjust=0.5, colour = "grey50"))+  xlab("BUSCO gene") + ylab("Species") + ggtitle(paste("Overview of complete single-copy BUSCOs per species (total genes=", as.character(total),") BUSCO set: ",busco_set, sep=""))


pdf(file=outfile1, width=mywidth, height=myheight)
print(p)
dev.off()
...
pdf(file=outfile2, width=mywidth)
print(p2)
dev.off()
