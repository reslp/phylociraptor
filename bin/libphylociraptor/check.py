...
def return_results_location(mode = "", hashes={}, debug=False, verbose=False):
	...
	if mode == "filter-orthology":
		out = "  ./results/orthology/single-copy-orthologs_deduplicated." + hashes[mode]["global"] + "\n"
		return out.rstrip()
	if mode == "align":
		out = ""
...
def check_is_running(mode="", previous_mode="", hashes={}, previous_hashes={}, debug=False, verbose=False):
	...
	if mode == "align":
		...
		if not os.path.isdir("results/alignments"):
			return
		for gene in os.listdir("results/orthology/single-copy-orthologs_deduplicated."+previous_hashes[previous_mode]["global"]+"/"):
