...
if (file.exists(downloaded_genomes_statistics_file))
{
  ...
  data %>% mutate(ftp_path = cell_spec("download", "html", link = ftp_path)) %>% kbl(escape=F) %>% kable_paper("hover", full_width = F) %>% scroll_box(width = "100%", height = "600px")
} else {
...
cat("<br><b> Trimming statistic files not found. Did you run phylociraptor filter-align?</b>\n")
	...
	for (i in 1:length(colnames(data_combined))){
		cat(paste0("Number of alignemnts for ", colnames(data_combined)[i], 1), "\n")
	}
