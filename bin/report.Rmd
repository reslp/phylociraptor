...
```{python getorthomethod, echo=FALSE}
orthomethod = config_data["orthology"]["method"]

```

```{r genomes, echo=FALSE}
# this information here is only need if BUSCO has been used
# because only then genomes could have been downloaded.
...
`r if(!py$orthomethod == "busco"){"\\begin{comment}"}`
...
`r if(!py$orthomethod == "busco"){"\\end{comment}"}`


```{r genome_stats, echo=F, out.height=500}

info <- ""
if (file.exists(downloaded_genomes_statistics_file) && py$orthomethod == "busco")
{
...
} else if (py$orthomethod == "orthofinder") {
  info <- "Setup for runmode orthofinder was successful."
...
```{python getorthologyhash, echo=FALSE}
...
`r if(py$orthomethod == "orthofinder") {"\\begin{comment}"}`
...
if (file.exists(busco_summary_file) && py$orthomethod != "orthofinder")
{	
...
```{r busco_overview, include=TRUE, fig.align="center", echo=F}
#out.width="1.0\\linewidth"
if (file.exists(busco_overview_image)) {
  include_graphics(busco_overview_image)
} 
```
`r if(py$orthomethod == "orthofinder") {"\\end{comment}"}`



`r if(py$orthomethod == "busco") {"\\begin{comment}"}`

**Location of Orthofinder results:**

```
results/orthology/orthofinder/orthofinder-`r py$orthology_hash`/orthofinder-results-`r py$orthology_hash`
```

```{r orthofinder_summary, results="asis", echo=FALSE}
# this is just a quick way to display some summary statistics for the orthofinder run. This needs to be improved!
orthofinder_file=paste0("../results/orthology/orthofinder/orthofinder-",py$orthology_hash, "/orthofinder-results-", py$orthology_hash, "/Comparative_Genomics_Statistics/Statistics_Overall.tsv")
...
if (file.exists(orthofinder_file)) {
	...
	cat("Orthofinder statistics file not found. Maybe Orthofinder was not used? <br>")
}
```

**Careful:** The number of single copy orthologs identified by Orthofinder may not correspond to the number in the alignment step. This is because phylociraptor allows for missing data while Orthofinder does not. For example a gene may be missing in one species but has a single copy in all other species. Such a gene would not show up as single copy ortholog in Orthofinder but it will still be included in phylociraptor given that the parameters are set accordingly.

`r if(py$orthomethod == "busco") {"\\end{comment}"}`

```{python detalitrim, echo=FALSE}
aligners = []
taligners = []
faligners = []
...
for trimmer in all_hashes["filter-align"]["filter-align"]["per-trimming"]:
	for aligner in all_hashes["filter-align"]["filter-align"]["per-trimming"][trimmer]:
		tf = "../results/statistics/trim-" + aligner + "-" + trimmer + "." + all_hashes["filter-align"]["filter-align"]["per-trimming"][trimmer][aligner]
...
```{python get_filter_hash, echo=FALSE}

filterhash = all_hashes["filter-align"]["filter-align"]["global"]
```

```{r ext-figures, echo=FALSE, fig.width = 11.7, message=FALSE, warning=FALSE}
f <- paste0("../results/statistics/filtered-alignments-stats.",py$filterhash,".txt")
if (identical(f, character(0)) == FALSE && file.exists(f) == TRUE) {
	...
	plotlist <- list()
	i <- 1
	#cols <- c("#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#cccccc", "#000000" , "#000000" ,"#000000" ,"#000000" ,"#000000" ,"#000000")
	# get unqiue number of models in dataset and create colors for them:
	combs <- length(unique(data$combination))
	cols <- c()
	if (combs <= 11) {
		# prettier colors when there are not too many different models
		cols <- brewer.pal(combs, "BrBG")		
	} else {
		# we need more colors
		color <- grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
		cols <- sample(color, combs)
	}
...
```{python getmodeltesthash, echo=FALSE}
faligners = []
gtfilter = []
...
```{python getsptreehash, echo=FALSE}
sptreestats = []
