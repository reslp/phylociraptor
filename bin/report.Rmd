...
# display of benchmark results need more work
...
cat("<br><b> Alignment statistic files not found. Did you run phylociraptor align?</b>\n")
	...
	cat(paste0("<b>Total number of alignments per aligner: </b>", nrow(data_combined), "<br><br>"))	
...
cat("<b> Trimming parameters:</b><br>")
parsimony_cutoff <- 100000 # in case this can not be determined from the overview stats file, use high value to flag nothing
if (file.exists(align_trim_overview_statistics_file) == TRUE) {
  ov <- read.csv(align_trim_overview_statistics_file, sep="\t", header=T, row.names=NULL)
  ov <- as.data.frame(ov)
  print(ov[,c(1,2,3,8)] %>% kbl(escape=F) %>% kable_paper("striped", full_width = F) %>% kable_styling())
  parsimony_cutoff <- as.numeric(ov[1,ncol(ov)])
} else {
...
cat("<b> Filtering parameters:</b><br>")
parsimony_cutoff <- 100000 # in case this can not be determined from the overview stats file, use high value to flag nothing
if (file.exists(align_trim_overview_statistics_file) == TRUE) {
  ov <- read.csv(align_trim_overview_statistics_file, sep="\t", header=T, row.names=NULL)
  ov <- as.data.frame(ov)
  print(ov[,c(1,2,4,5,6,7,8)] %>% kbl(escape=F) %>% kable_paper("striped", full_width = F) %>% kable_styling())
  parsimony_cutoff <- as.numeric(ov[1,ncol(ov)])
} else {
  tr_method <- "unknown (check if phylocraptor -m align has finished successfully)."
  tr_params <- numeric()
...
```{r stats_speciestree, message=FALSE, echo=FALSE, warning=FALSE, results="asis", fig.align="center"}
files <- list.files(path="../results/statistics/", pattern ="genetree_filter_*", recursive=F)
multi_merge <- function(d) {
	df <- d[[1]]
	if (length(d) >= 2) {
		for (i in 2:length(dat)) {
...
if (length(files) > 0) {
	...
	all_colnames <- c("gene", "aligner", "trimmer", "bootstrap_sum", "bootstrap_n", "bootstrap_mean", "bootstrap_cutoff", "check")
	titles <- c()
	plots <- list()
	models <- c()
	thresh <- 0
	...
	for (file in files) {
		...
		title[2] <- strsplit(title[2], ".", fixed=T)[[1]][1]
		title <- paste(title, collapse="-")
 		names <- c(names,  c("gene", "bootstrap_mean", "check"))
		titles <- c(titles, title)
		dat[[i]] <- read.table(paste0("../results/statistics/", file), sep="\t", header=F)
  		colnames(dat[[i]]) <- all_colnames 
  		thresh <- dat[[i]]$bootstrap_cutoff[1]
		dat[[i]] <- dat[[i]][,c("gene", "bootstrap_mean", "check")]
	...
	plot_data$gene <- data_combined$gene
	plot_data <- melt(plot_data)
	colnames(plot_data) <- c("gene", "aligner_trimmer", "bootstrap_mean") 
	p <- ggplot(plot_data, aes(x=aligner_trimmer, y=bootstrap_mean)) + geom_violin() + geom_hline(yintercept=thresh, color="red", alpha=0.5) + labs(title ="Distribution of mean bootstrap values for gene trees", x = "Aligner and Trimmer combination",y= "Mean Bootstrap values")
	print(p)
	cat("<br>\n")
	for (s in genes_above_thresh_outstring) {
		cat(s)
	}
		
	cat("<br><b>Overview of mean bootstrap values per gene-tree: </b><br>\n")
	options(knitr.kable.NA = '') # do not display NA values
	data_combined %>% kbl(escape=F) %>% add_header_above(get_headers(titles)) %>% kable_paper("striped", full_width = F)%>% scroll_box(width = "100%", height = "600px")	
	#data_combined %>% kbl(escape=F) %>% kable_paper("striped", full_width = F)%>% scroll_box(width = "100%", height = "600px")	
