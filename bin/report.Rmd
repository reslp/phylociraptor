...
# 1. There is a problem with trimmed and filtered alignment numbers. Something seems to be off with the stats files...
...
cat("<br><b> Trimming statistic files not found. Did you run phylociraptor filter-align?</b>\n")
	...
	i <- 1
	for (dir in dirs) {
		files <- list.files(path=dir, pattern="*stats*", full.names=T)
		data <- do.call(rbind,lapply(files,read_my_csv))	
