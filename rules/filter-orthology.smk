...
rule read_params_global:
	input:
		trigger = compare("results/orthology/parameters.filter-orthology."+current_hash+".yaml", configfi),
		previous = "results/orthology/parameters.orthology."+previous_hash+".yaml"
	output:
...
if config["orthology"]["method"] == "busco":
	rule create_sequence_files:
		...
		output:
			sequence_dir=directory("results/orthology/single-copy-orthologs."+current_hash+"/{batch}-"+str(config["concurrency"])),
	...
	rule remove_duplicated_sequence_files:
		...
		shell:
			"""
			if [[ -d results/orthology/single-copy-orthologs_deduplicated.{params.current_hash} ]]; then
				rm -rf results/orthology/single-copy-orthologs_deduplicated.{params.current_hash}
			fi
			mkdir results/orthology/single-copy-orthologs_deduplicated.{params.current_hash}
			...
			for file in results/orthology/single-copy-orthologs.{params.current_hash}/*/*.fas;
				do
				if [[ {params.dupseq} == "persample" ]];
				then
					# per sequence filtering
					python bin/filter_alignments.py --alignments {params.wd}/$file --outdir "{params.wd}/results/orthology/single-copy-orthologs_deduplicated.{params.current_hash}" --per_sample --minsp {params.minsp} >> {output.statistics}
				else
					# whole alignment filtering
					python bin/filter_alignments.py --alignments {params.wd}/$file --outdir "{params.wd}/results/orthology/single-copy-orthologs_deduplicated.{params.current_hash}" >> {output.statistics}
				fi
			done
			...
			echo "$(date) - Number of BUSCO sequence files: $(ls results/orthology/single-copy-orthologs.{params.current_hash}/*/*.fas | wc -l)" >> results/statistics/runlog.txt
			echo "$(date) - Number of deduplicated BUSCO sequence files: $(ls results/orthology/single-copy-orthologs_deduplicated.{params.current_hash}/*.fas | wc -l)" >> results/statistics/runlog.txt
			touch {output.checkpoint}
			"""
else:
	rule create_sequence_files:
		...
		benchmark:
			"results/statistics/benchmarks/create_seq_files/create_sequence_files_1-1." + current_hash + ".txt"
		params:
			cutoff=config["filtering"]["cutoff"],
			minsp=config["filtering"]["minsp"],
			of_dir = "results/orthology/orthofinder/orthofinder-"+previous_hash + "/orthofinder-results-"+previous_hash+"/Orthogroup_Sequences/",
			seqtype = config["filtering"]["seq_type"],
			exclude = config["filtering"]["exclude_orthology"],
			nbatches = 1
		singularity:
			containers["biopython"]
		log: "log/filter-orthology/exSeqfiles_1-1." + current_hash + ".log"
		shell:
			""" 
			if [[ ! -d {output.sequence_dir} ]]; then mkdir -p {output.sequence_dir}; fi
			exclude=""
			...
			python bin/create_sequence_files.py --mode orthofinder --type {params.seqtype} --orthology_table {input.table} --orthology_results {params.of_dir} --cutoff {params.cutoff} --outdir {output.sequence_dir} --minsp {params.minsp} --genome_statistics {output.genome_statistics} --gene_statistics {output.gene_statistics} --batchID {params.nbatches} --nbatches {params.nbatches} --fix_aa_codes $exclude 
		
			#&> {log}
			touch {output.checkpoint}   
			touch {output.dubseq_check}
			"""
